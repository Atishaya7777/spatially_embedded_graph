...
\begin{frame}{This week}
	...
	\begin{itemize}
		\item Read through and understand the weak NP-Hardness reduction.
		      \pause
		\item Think of suitable gadgets to prove the unbounded version is also NP-Hard.
	\end{itemize}
\end{frame}
...
\begin{frame}{Why is the bound $wt(T) \leq B$ important?}
	...
	\begin{itemize}
		\item We could just have everything open, which might reduce the Wiener index (Since open gadgets keep gadget paths short), but increase the wieght
...
\begin{frame}{Next steps?}
	Construct spanning tree $T$ of point set $P$ and show:
	\begin{itemize}
		\item If \textbf{PARTITION} is solvable, then $W(T) \leq W$.
		\item Otherwise, ALL trees have Wiener index $> W + \delta$ for some $\delta > 0$.
	\end{itemize}
\end{frame}

\begin{frame}{Next week tasks!}
	\begin{itemize}
		\item CCCG/WADS (Yay)! Read the pre-prints on arxiv and see which ones I think I would enjoy listening in the most.
		\item Keep thinking about the gadgets that we could use.
	\end{itemize}
\end{frame}
