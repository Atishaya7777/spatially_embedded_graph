    ...
    def calculate_wiener_index(path: List[Point]) -> float:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                forward_dist = WienerIndexCalculator._cycle_distance(
                    path, i, j)
                # Distance going backward in cycle
                backward_dist = WienerIndexCalculator._cycle_distance(
                    path, j, i)
...
class BruteForceSolver:
    ...
    def _parallel_solve(self, fixed_point: Point, remaining_points: List[Point]) -> Tuple[List[Point], float]:
        ...
        chunks = [perms[i:i + chunk_size]
                  for i in range(0, len(perms), chunk_size)]

        logging.info(f"Evaluating {len(perms)} permutations using {
                     num_workers} workers...")
        ...
        logging.info(f"Parallel brute force completed in {
                     elapsed:.2f} seconds")
...
class DivideConquerSolver:
    ...
    def _divide_conquer_recursive(self, points: List[Point], use_median: bool, depth: int) -> List[Point]:
        ...
        left_path = self._divide_conquer_recursive(
            left_group, use_median, depth + 1)
        right_path = self._divide_conquer_recursive(
            right_group, use_median, depth + 1)
    ...
    def run_comprehensive_study(self, point_counts: List[int], point_types: List[str],
        ...
        if algorithms is None:
            algorithms = ['brute_force',
                          'divide_conquer', 'divide_conquer_median']

        all_results = []
        total_experiments = len(point_counts) * len(point_types) * num_trials

        self.logger.info(f"Starting comprehensive study: {
        ...
        self.logger.info(f"Comprehensive study completed: {
                         len(all_results)} experiments")
        return all_results
...
class Visualizer:
    ...
    def plot_experiment_comparison(self, experiment: ExperimentResult, save_path: str = None) -> None:
        ...
        for idx, (alg_name, result) in enumerate(experiment.results.items()):
            ...
            ax.scatter(x_coords, y_coords, c='black',
                       s=60, alpha=0.7, zorder=3)

            # Plot path if available
            if result.path:
                path_x = [p.x for p in result.path] + \
                    [result.path[0].x]  # Close the cycle
    ...
    def plot_performance_summary(self, results: List[ExperimentResult], save_path: str = None) -> None:
        ...
        fig.suptitle('Wiener Index Analysis Performance Summary',
        ...
        for alg in algorithms:
            ...
            ax1.plot(point_counts, avg_wieners,
        ...
        for alg in algorithms:
            ...
            ax2.plot(point_counts, avg_times,
        ...
        if 'brute_force' in algorithms:
            for alg in algorithms:
                ...
                ax3.plot(point_counts, ratios_by_count,
        ...
        for alg in algorithms:
            ...
            if ratios:
                ...
                print(f"  Optimal solutions: {
                      sum(1 for r in ratios if r <= 1.001)} / {len(ratios)}")
                print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Wiener Index Analysis Framework')
    ...
    try:
        ...
        for i, result in enumerate(example_results):
            example_plot_path = orchestrator.output_dir / \
                f"experiment_example_{i+1}.png"
            visualizer.plot_experiment_comparison(
                result, str(example_plot_path))
